...
def prior(name, y_onehot, hps):
    ...
    with tf.variable_scope(name):
        ...
        if hps.ycond:
            h += tf.reshape(Z.linear_zeros("y_emb", y_onehot, 2*n_z), [-1, 1, 1, 2 * n_z])
...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    def _f_loss(x, y, is_training, reuse=False):
        ...
        with tf.variable_scope('model', reuse=reuse):
            ...
            # Generative loss
            nobj = - objective
            bits_x = nobj / (np.log(2.) * int(x.get_shape()[1]) * int(x.get_shape()[2]) * int(x.get_shape()[3]))  # bits per subpixel
            ...
            if hps.weight_y > 0 and hps.ycond:
                ...
                bits_y = tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_onehot, logits=y_logits) / np.log(2.)

                # Classification accuracy
                y_predicted = tf.argmax(y_logits, 1, output_type=tf.int32)
                classification_error = 1-tf.cast(tf.equal(y_predicted, y), tf.float32)
            else:
    ...
    def f_loss(iterator, is_training, reuse=False):
        ...
        global_stats = Z.allreduce_mean(tf.stack([tf.reduce_mean(i) for i in stats]))
...
def invertible_1x1_conv(name, z, logdet, reverse=False):
    ...
    if True: # Set to "False" to use the LU-decomposed version
        ...
        with tf.variable_scope(name):
            ...
            import scipy
            np_w = scipy.linalg.qr(np.random.randn(shape[3], shape[3]))[0].astype('float32')
            ...
            if not reverse:
                ...
                z = tf.nn.conv2d(z, w_inv, [1,1,1,1], 'SAME', data_format='NHWC')
                logdet -= tf.reduce_sum(log_s) * (shape[1]*shape[2])
