...
if __name__ == "__main__":
    ...
    parser.add_argument("--realnvp", action='store_true', help="Preprocess according to realnvp")
    parser.add_argument("--res", type=int, default=256, help="Image size")
    parser.add_argument("--lsun_dir", type=str, required=True, help="place of lsun dir")
    ...
    for realnvp in [False, True]:
        for category in ["tower", "church_outdoor", "bedroom"]:
            ...
            if realnvp:
                hps.tfrecord_dir = "%s_%s/%s" % (base_tfr, "realnvp", hps.category)
            else:
