...
def batch_downsample(x, factor):
    shape = tf.shape(x)
    batch, height, width, channels = shape[0], shape[1], shape[2], shape[3]
    x = tf.reshape(x, [batch, height // factor, factor, width // factor, factor, channels])
...
def pillow_preprocess_image(image, max_res, is_training=False):
    min_side = min(image.shape[0], image.shape[1])
    hs = np.random.randint(0, image.shape[0] - min_side + 1)
    ws = np.random.randint(0, image.shape[1] - min_side + 1)
    image = Image.fromarray(image)
    image = image.crop((hs,ws,hs + min_side,ws + min_side))
    image = image.transpose(Image.FLIP_LEFT_RIGHT)
    full_res_image = image.resize((max_res, max_res), resample=Image.BILINEAR)
    return full_res_image

def preprocess_image(image, max_res, is_training=False):
...
def downsample_images(full_res_images, labels, shape, initial_factor, final_factor = 2, full_res = False):
    full_res_images.set_shape(shape)
    full_res_images = tf.cast(full_res_images, 'float32')
    batch, _, _, num_channels = full_res_images.get_shape().as_list()
    downsample = fast_batch_downsample

    # Downsample to get target images
    images = downsample(full_res_images, initial_factor)

    # Further downsample
    downsampled_images = downsample(images, final_factor)
    downsampled_images = x_to_uint8(downsampled_images)

    if full_res:
        full_res_images = x_to_uint8(full_res_images)
        return full_res_images, downsampled_images, labels
    else:
        images = x_to_uint8(images)
        return images, downsampled_images, labels # _mean_image_subtraction(image, [_R_MEAN, _G_MEAN, _B_MEAN])