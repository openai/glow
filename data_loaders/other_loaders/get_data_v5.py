...
def parse_tfrecord_tf(record, res, lowest, factor):
...
def input_fn(tfr_file, shards, rank, pmap, fmap, n_batch, resolution, is_training, lowest, factor = 2):
    ...
    dset = dset.repeat()
    dset = dset.map(lambda x: parse_tfrecord_tf(x, resolution, lowest, factor), num_parallel_calls= pmap)
